...
TOOL_CREATION_DIRECTIVE = (
    ...
    "COMPOSE — inside a tool you write, EVERY other tool is already available to you as a plain "
    "function. Call it by name with a dict of its arguments; it returns that tool's output as a "
    "STRING, so json.loads() it when the tool returns JSON. For example, to get coordinates:\n"
    "    place = json.loads(geocode({'location': args['location']}))\n"
    "    lat, lon = place['latitude'], place['longitude']\n"
    "This is the FIRST thing to reach for when your tool needs something a built-in already does. "
    "Do NOT re-implement a built-in against a raw public API, and NEVER hardcode a value you got "
    "from calling a tool (e.g. pasting a latitude into the code) — that tool works for every input, "
    "and your hardcoded copy works for exactly one. If a built-in resolves a place name that the "
    "raw API cannot, call the built-in. "
